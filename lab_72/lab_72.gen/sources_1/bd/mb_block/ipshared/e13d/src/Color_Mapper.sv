...
module  color_mapper ( input  logic [9:0] DrawX, DrawY, 
    ...
    I could actually reuse the font rom for text for the timer and other things, but that is far in the future
    ...
    */
    ...
    // (The RGB_Display just lets us know what the comb block
    
    always_comb begin:RGB_Display
    
        // Cube bounds
        if ( 180 <= DrawX && DrawX <= 460 && 100 <= DrawY && DrawY <= 380)
        begin
            // Assign the colors based on the input colors for this range
            Red   = in_color[11:8];
            Green = in_color[7:4];
            Blue  = in_color[3:0];
        end
        
        // Put other sections here as you add them
        
        else // Background
        begin
            // If not in the middle, color teal
            Red   = 4'b0;
            Green = 4'b1100;
            Blue  = 4'b1100;
        end
