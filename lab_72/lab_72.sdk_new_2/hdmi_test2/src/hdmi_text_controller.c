...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
    ...
    // hdmi_ctrl->color_pallete[color/2] = 0xFFFF; hard coding values to white debug

    // volatile uint32_t *base = (uint32_t)(hdmi_ctrl + 0x800); // Sets the base to the start of the color_pallete

    uint8_t reg_index = color & 0x7;
    uint8_t is_odd  = (color/8); //color >> 3

    uint32_t pal_val = hdmi_ctrl->PALETTE_REGS[reg_index];
    uint32_t color_combo = ((red & 0xF) << 8) | ((green & 0xF) << 4) | ((blue  & 0xF));
    if (is_odd) {
        pal_val = (pal_val & ~(0xFFF << 16)) | (color_combo << 16);
    } else {
        pal_val = (pal_val & ~0xFFF) | color_combo;
    }

    hdmi_ctrl->PALETTE_REGS[reg_index] = pal_val;
